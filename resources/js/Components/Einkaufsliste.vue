        ...
        <div class="flex-1">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <small v-if="liste.created_at !== liste.updated_at" class="text-sm text-gray-600"> &middot; bearbeitet</small>
                </div>
...
<template #content>
    ...
    <button class="block w-full px-4 py-2 text-left text-sm leading-5 text-gray-700 hover:bg-gray-100 focus:bg-gray-100 transition duration-150 ease-in-out" @click="editing = true">
        ...
        Bearbeiten
            ...
            <form v-if="editing" @submit.prevent="form.put(route('listen.update', liste.id), { onSuccess: () => editing = false })">
...
<p v-else class="mt-4 text-lg text-gray-900">{{ liste.message }}</p>
        </div>
    </div>
</template>